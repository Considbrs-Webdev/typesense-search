...
class SearchKeyActions
{
    ...
    {
        $this->requirePermission(SettingsAjax::AJAX_ACTION_GEN_KEY);
        ...
        // The provisioning key is never sent to a form-supplied/unsaved remote:
        // the destination is always the already-saved connection setting.
        $remote = $this->settings->getRemote();

        if (empty($remote)) {
            wp_send_json_error([
                'message' => __('Save the host under Server before generating a search key automatically, or enter a search key manually below.', 'typesense-search'),
            ]);
            return;
        }

        try {
            $client = ProvisioningClientFactory::fromTrustedRemote($remote);
    ...
    public function handleFixSearchKey(): void
    {
        ...
        if (empty($remote) || empty($collectionName)) {
            wp_send_json_error([
                'message' => __('Host and collection name must both be saved before a new search key can be created.', 'typesense-search'),
            ]);
            return;
        }
