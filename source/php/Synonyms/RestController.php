...
/**
 * Admin-only REST API for synonym rules.
 */
class RestController
{
    public const REST_NAMESPACE = 'typesense-search/v1';
    public const REST_ROUTE = '/synonyms';
    ...
    public function save(WP_REST_Request $request): WP_REST_Response|\WP_Error
    {
        ...
        $terms = is_array($body['terms'] ?? null) ? (array) $body['terms'] : [];

        $saved = $this->repository->save($id ?: null, $terms);
    ...
    {
        if (!$this->settings->isSynonymsEnabled() || !$this->capabilities->supportsSynonymSets()) {
            $this->sendNoCacheHeaders();

            return new \WP_Error(
                'synonyms_unavailable',
                __('Synonyms are not enabled or this Typesense server does not support synonym sets.', 'typesense-search'),
                ['status' => 403]
            );
        }
    ...
    private function sendNoCacheHeaders(): void
    {
        ...
        do_action('litespeed_control_set_nocache', 'Typesense synonyms REST');
    }
